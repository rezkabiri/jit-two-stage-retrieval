...
2.  **`VERTEX_AI_MODEL` / `AI_STUDIO_MODEL` (Configuration)**: Independent environment variables that specify which model to use for each provider. This allows us to point the system to a specific versioned model (e.g., `gemini-2.5-pro`) or an unversioned alias (e.g., `gemini-2.5-pro`).
